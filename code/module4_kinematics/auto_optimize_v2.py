...
def main():
    ...
    reachable = []
    best_joints_list = []
    for i, pt in enumerate(drill_points):
    ...
    print(f"\n可达性结果: {num_ok}/{len(drill_points)} ({num_ok/len(drill_points)*100:.1f}%)")
    ...
    arm_labeled = False
    for i, (pt, ok, joints) in enumerate(zip(drill_points, reachable, best_joints_list)):
    ...
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Z (m)')
    ax.set_title(f'Reachability (Truck at {TRUCK_DISTANCE}m) | {num_ok}/{len(drill_points)} reachable')
